<template>
    <v-card title="My homework" :disabled="componentLoading" :loading="componentLoading">
        <template v-slot:append>
            <v-btn to="/student/homeworks" icon="mdi-open-in-new" variant="text"> </v-btn>
        </template>
        <v-card-text v-if="!componentLoading">
            <v-list v-if="homeworks.length > 0">
                <div v-for="(homework, index) in homeworks" :key="index">
                    <v-list-item :variant="homework.is_done ? 'plain' : 'flat'" :key="index">
                        ...
                        <template v-slot:append>
                            <v-btn icon variant="text" :to="`/student/homeworks?view=${homework.homework_id}`">
                                <v-icon>mdi-eye</v-icon>
                            </v-btn>
                        </template>
                        <v-list-item-title
                            >{{ homework.homework_title }} {{ homework.is_done ? "(Done)" : "" }}</v-list-item-title
                        >
                        <v-list-item-subtitle
                            >Due in {{ homework.homework_due_date.toLocaleDateString("fr-FR") }}</v-list-item-subtitle
                        >
                    </v-list-item>
                    <v-divider v-if="index < homeworks.length - 1"></v-divider>
                </div>
            </v-list>
            <p v-else class="text-center text-grey">There is no homeworks to display</p>
        </v-card-text>
        <v-card-text v-else>
            <v-skeleton-loader :loading="componentLoading" type="table-heading"></v-skeleton-loader>
            <v-divider />
            <v-skeleton-loader :loading="componentLoading" type="table-heading"></v-skeleton-loader>
            <v-divider />
            <v-skeleton-loader :loading="componentLoading" type="table-heading" class="mb-4"></v-skeleton-loader>
        </v-card-text>
