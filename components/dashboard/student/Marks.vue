<template>
    <v-card title="My recent marks" :disabled="componentLoading" :loading="componentLoading">
            ...
            <v-list v-if="marks.length > 0">
                <div v-for="(mark, index) in marks" :key="index">
                    <v-list-item>
                        ...
                        <v-list-item-title
                            >{{ mark.exam_title }} <span class="text-body-2">({{ mark.exam_unit }})</span></v-list-item-title
                        >
                        <v-list-item-subtitle
                            >Added the {{ new Date(mark.exam_date).toLocaleDateString("fr-FR") }}</v-list-item-subtitle
                        >
                    </v-list-item>
