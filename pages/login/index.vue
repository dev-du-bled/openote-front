<template>
    <v-container>
        <v-form style="max-width: 400px; width: 100%; margin: 0 auto" @submit.prevent="login">
            <v-alert v-if="errorMsg" closable type="error" class="mb-2">{{ errorMsg }}</v-alert>
            <v-card title="Login" subtitle="Login to your OpeNote account" :loading="loading" :disabled="loading">
                <v-card-text>
                    ...
                    />
                    <v-checkbox v-model="longerSession" label="Stay logged in longer" hide-details />
                </v-card-text>
        ...
        <v-card title="Login as (Debug only)" style="margin: 0 auto; max-width: 400px" class="mt-3">
            <v-card-actions>
                <v-btn
                    @click="
        ...
        <v-dialog v-model="forgotPasswordDialog" max-width="750px">
            <v-card :loading="loading" :disabled="loading">
                ...
                <v-card-text>
                    <v-text-field v-model="forgotPasswordEmail" label="Email" type="email" required />
                </v-card-text>
                <v-card-actions class="bg-surface-light">
