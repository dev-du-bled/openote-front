<template>
    <v-container>
        <v-card title="Homeworks" :disabled="componentLoading" :loading="componentLoading">
            <template v-slot:append>
                ...
                <v-tooltip text="Create a self homework">
                    <template v-slot:activator="{ props }">
                        <v-btn v-bind="props" @click="addSelfHomeworkDialog = true" icon="mdi-plus" variant="text"></v-btn>
                    </template>
                </v-tooltip>
            </template>
            <v-card-text>
                <v-row>
                    <v-col cols="12" md="6">
                        <v-list>
                            ...
                            <div v-for="(homework, index) in homeworks.filter((hw) => !hw.is_done)" :key="index">
                                <v-list-item
                                    :prepend-icon="homework.is_author ? 'mdi-account-school' : ''"
                                    :variant="selectedHomework?.homework_id === homework.homework_id ? 'tonal' : 'flat'"
                                    ...
                                    <v-list-item-subtitle
                                        :class="
                                            homework.homework_due_date < new Date()
                                                ? 'text-red-accent-4'
                                                : homework.homework_due_date.getTime() - new Date().getTime() < 86400000
                                                  ? 'orange-darken-4'
                                                  : ''
                                        "
                                    >
                                ...
                                <v-divider v-if="index < homeworks.filter((hw) => !hw.is_done).length - 1"></v-divider>
                            </div>
                        ...
                        <v-list v-if="homeworks.filter((hw) => hw.is_done).length > 0">
                            ...
                            <div v-for="(homework, index) in homeworks.filter((hw) => hw.is_done)" :key="index">
                                <v-list-item
                                    :prepend-icon="homework.is_author ? 'mdi-account-school' : ''"
                                    :variant="selectedHomework?.homework_id === homework.homework_id ? 'tonal' : 'plain'"
                                    @click="selectHomework(homework)"
                                >
                                    <v-list-item-title> {{ homework.homework_title }} (Done) </v-list-item-title>
                                    <v-list-item-subtitle>
                                        Due in
                                        {{ homework.homework_due_date.toLocaleDateString("fr-FR") }}
                                    </v-list-item-subtitle>
                    ...
                    <v-col cols="12" md="6">
                        <div v-if="selectedHomework">
                            ...
                            <p>Status: {{ selectedHomework.is_done ? "Done" : "Not Done" }}</p>
                            <p
                                :class="
                                    selectedHomework.homework_due_date < new Date()
                                        ? 'text-red-accent-4'
                                        : selectedHomework.homework_due_date.getTime() - new Date().getTime() < 86400000
                                          ? 'orange-darken-4'
                                          : ''
                                "
                            >
        ...
        <v-dialog v-model="addSelfHomeworkDialog" max-width="750px">
            <v-card title="Add a self homework">
                <v-card-text>
                    <v-text-field v-model="newHomeworkTitle" label="Title" required></v-text-field>
                    <v-textarea v-model="newHomeworkDetails" label="Details" required></v-textarea>
                    <v-date-input
                ...
                <v-card-actions class="bg-surface-light">
                    <v-btn variant="text" @click="addSelfHomeworkDialog = false">Cancel</v-btn>
                    <v-btn color="primary" @click="addSelfHomework">Add</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        <v-snackbar color="error" timer="#730800" :text="errorSnackbar" v-model="isErrorSnackbarVisible"></v-snackbar>
    </v-container>
</template>
    ...
    await $fetch(`${config.public.api_base_url}/homework/`, {
        ...
        .then((data) => {
            ...
            if (router.currentRoute.value.query.view) {
                const homework = homeworks.value.find((hw) => hw.homework_id === Number(router.currentRoute.value.query.view));
                if (homework) {
