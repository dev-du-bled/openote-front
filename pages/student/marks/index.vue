<template>
    <v-container>
        <v-card title="Marks" :disabled="componentLoading" :loading="componentLoading">
            <v-card-text>
...
const groupedMarks = computed(() => {
    const grouped = marks.value.reduce(
        (acc, mark) => {
    ...
    for (const unit in grouped) {
        grouped[unit].sort((a, b) => new Date(b.exam_date).getTime() - new Date(a.exam_date).getTime());
    }
